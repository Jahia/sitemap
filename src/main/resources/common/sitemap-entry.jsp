...
<url>
    ...
    <c:set var="localizedUrl" value="${url.context}${urlNode.url}"/>
    <c:url var="localeUrl" value="${localizedUrl}"/>
    <c:set var="goodLocalUrl" value="${canUseVanity ? localeUrl : localizedUrl}"/>
    ...
    <c:if test="${locales.size() > 1}"><%-- no need for alt if there's only one locale --%>
        <c:forEach var="locale" items="${locales}">
            ...
            <c:set var="localizedUrl" value="${url.context}${fn:replace(urlNode.url, languageToReplacePart, langReplacement)}"/>
            <c:url var="localeUrl" value="${localizedUrl}"/>
